...
resource "aws_subnet" "list_subnet" {
  vpc_id            = aws_vpc.vpc.id
  cidr_block        = "10.0.200.0/24"
  availability_zone = var.us-west-2-azs[0]
}
```

Run a `terraform plan` again. Check out the output and notice that the new subnet will be created in `us-west-2a`, because that is the first string in our list of strings. If we used `var.us-west-2-azs[1]` in the configuration, Terraform would have built the subnet in `us-west-2b` since that's the second string in our list.
...
{
  "dev" = {
    "arn" = "arn:aws:ec2:us-west-2:1234567890:subnet/subnet-052d26040d4b91a51"
