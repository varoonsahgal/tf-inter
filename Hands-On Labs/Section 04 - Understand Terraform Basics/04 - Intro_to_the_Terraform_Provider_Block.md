...
This informs Terraform that it will deploy services into the `us-west-2` region within AWS.
...
Once the infrastructure has been from our configured region, (in the example's case `us-west-2`), we can modify the `region` argument in the `provider` block to specify a different region.
